...
#%% thresholding
zoom = 4
dataf = fftshift(fft2((data)* exp(2j*pi*(v_est[0]*fdr*K*M + a_est[0]*fa*K*M*M + a_est[0]*frs*M*M)), [zoom*m, zoom*k]), -1)
...
dataf = fftshift(fft2((data)* exp(2j*pi* ( v_est[1]*fdr*K*M + a_est[1]*fa*K*M*M + a_est[1]*frs*M*M)), [zoom*m, zoom*k]), -1)
fig = plt.figure(figsize=[16,5])
fig.add_subplot(1, 2, 1)
data_fft2_db = 20*log10(abs(dataf))
plt.imshow(np.flipud(data_fft2_db).T,
